...
In this example, we've used the `theme_bw()` function to apply a black-and-white theme to the plot.  Some other theme functions that can be used to modify plot themes are -
...
Example-
```
png(filename = "scatter.png", width = 5, height = 3, units = "in", res = 300)
ggplot(mtcars, aes(x = wt, y = mpg)) + 
  geom_point() +
  geom_abline(intercept = 5, slope = 3, color = "seagreen")
dev.off()

pdf(file = "two_page.pdf", width = 6, height = 4)
...
### Indian States filled map (choropleth)
