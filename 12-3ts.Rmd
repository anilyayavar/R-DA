...
### Naive method
This method assumes that the future value will be the same as the last observed value. This is the simplest and most basic forecasting method. The formula for the naive method is:
...
### Moving averages
In this technique, the forecast value is computed as the average of the most recent observations within a sliding window of fixed length.  It is a technique to smooth out a time series by averaging neighboring values. It helps in reducing noise and revealing underlying trends. 
...
```{r ma1, warning=FALSE, message=FALSE, echo=FALSE, fig.align='center', fig.cap="Simple Moving average", fig.align='center'}
library(forecast)
library(zoo)
...
The value of $\alpha$, the smoothing parameter, in above equation should follow - $0 \le \alpha \le 1$.  Clearly different values of $\alpha$ will give different smoothing and we will have to adopt most suitable one, one such example with $\alpha = 0.1$ is shown in the chart \@ref(fig:ses1). 
```{r ses1, echo=FALSE, fig.cap="Simple Exponential Smoothing and Forecasts", fig.align='center'}
lynxses <- ses(lynx, h= 10, alpha = 0.1)

autoplot(lynxses) +
  autolayer(lynxses$fitted) +
  ggplot2::ggtitle('SES with alpha = 0.1')
```

Holt extended SES by introducing trend component and thus a new smoothing parameter for trend $\beta$. The method is also named after him as **Holt's Linear Method** of exponential smoothing. Mathematically, there are are now two equations.
$$
...
$\alpha$ and $\beta$ (again $0 \le \beta \le 1$) in above equations are smoothing parameters for level and trend components, respectively; and $h$ is forecast horizon. Also, $y_t$ is the actual value, $l_t$ is the level (or intercept), $b_t$ is the trend (or slope) of the time series at time $t$.  Refer \@ref(fig:hl).
```{r hl, echo=FALSE, fig.cap="Holt's Linear Method"}
holtfit <- holt(airmiles, h = 10, alpha = 0.3, beta = 0.1)
autoplot(holtfit) +
  autolayer(holtfit$fitted) +
  ggplot2::ggtitle("Holt's Linear Trend method with beta = 0.1")
```

As stated above, damped trends work better many times.  So based on Mckenzie and gardner work, an additional dampening paramater $\phi$ was introduced.  The equations were modified as 
$$
\begin{aligned}
...
Claerly, if $\phi = 1$, the above equations are equivalent to Holt's Linear Method. $\phi$ for values $0 < \phi < 1$, dampens the trend, as shown in figure \@ref(fig:hl2)

```{r hl2, echo=FALSE, fig.cap="Damped trend method"}
...
A comparison of both can be sen in the figure \@ref(fig:hl3).
```{r hl3, message=FALSE,warning=FALSE,echo=FALSE, fig.cap="Linear Method Vs. damped trends"}
library(ggplot2)
holtfit <- holt(airmiles, h = 10, alpha = 0.3, beta = 0.1)
...
\end{aligned}
$$
An example can be seen in figure \@ref(fig:hw1).

```{r hw1, echo=FALSE, fig.cap="Holt-Winter's Method of Forecasting"}
...
```{r stl1, echo=FALSE, fig.cap="Seasonal Decomposition using classical approach"}
AirPassengers |>
  decompose() |>
  plot()
```
...
A plot of STL decomposed `AirPassengers` is shown in figure \@ref(fig:stl2).
```{r stl2, echo=FALSE, fig.cap="STL Decomposition"}
...
In R, we can apply moving averages using the `ma()` function from `forecast` library.  
...
2. **Moving average method:** The moving average method is a simple technique, already explained above, that involves calculating the average of the past few observations. Example
